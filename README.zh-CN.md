...
- **双层记忆** — `MEMORY.md` 中的持久知识 + `memory/daily/` 中的每日事件日志。预压缩 flush 任务（01:45）在夜间压缩（02:00）之前保存当日上下文，压缩任务自动精简日志并提升持久性事实。对话中消息数量超过阈值时也会自动触发记忆整理。
- **会话管理** — 基于 JSONL 的会话持久化，支持可配置的 TTL 和自动过期回收，以及 `/new` 命令将当前对话归档到每日记忆后开启全新会话。
...
- **定时任务** — 内置 cron 调度器，支持心跳检查、记忆整理、记忆压缩和自定义周期性任务。
